#!/usr/bin/env python3
"""Run Cargo checks with the same local-toolchain discovery as desktop builds."""
...
def main() -> int:
    ...
    if not shutil.which("cargo", path=env.get("PATH")):
        raise SystemExit(
            "Cargo is unavailable. Install Rust or provide the project-local .tools toolchain."
        )
